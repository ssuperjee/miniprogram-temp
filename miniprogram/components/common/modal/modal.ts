...
Component({
  ...
  methods: {
    ...
    onKeyboardHeightChange(event: WechatMiniprogram.CustomEvent<{ height: number; duration: number }>) {
    ...
    clearLeaveTimer() {
      const timer = (this as any)._leaveTimer as ReturnType<typeof setTimeout> | undefined
      if (timer) {
        clearTimeout(timer)
